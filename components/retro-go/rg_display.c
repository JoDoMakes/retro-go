...
#define ILI9341_CMD(cmd, data...) {const uint8_t x[] = data; ili9341_cmd(cmd, x, sizeof(x));}
    ...
    ILI9341_CMD(0xE8, {0x85, 0x10, 0x79});
    ILI9341_CMD(0xCB, {0x39, 0x2c, 0x00, 0x34, 0x02});
    ILI9341_CMD(0xF7, {0x20});
    ILI9341_CMD(0xEA, {0x00, 0x00});
    ILI9341_CMD(0xC0, {0x22});                                  // Power control   //VRH[5:0]
    ILI9341_CMD(0xC1, {0x11});                                  // Power control   //SAP[2:0];BT[3:0]
    ILI9341_CMD(0xC5, {0x3a, 0x1C});                            // VCM control
    ILI9341_CMD(0xC7, {0xa9});                                  // VCM control2
    ILI9341_CMD(0x36, {(0x20|0x40|0x08)}); // 0x80 0x40 0x20 0x10                       // Memory Access Control
    ILI9341_CMD(0x3A, {0x55});                            // Frame Rate Control (1B=70, 1F=61, 10=119)
    ILI9341_CMD(0xB1, {0x00, 0x1B});                            // Display Function Control
    ILI9341_CMD(0xB6, {0x0A, 0xA2});
    ILI9341_CMD(0xF6, {0x01, 0x1D});                                  // 3Gamma Function Disable
    ILI9341_CMD(0xF2, {0x00});     
    ILI9341_CMD(0x26, {0x01});                                  // Gamma curve selected

    ILI9341_CMD(0xE0, {0x0F, 0x3F, 0x2F, 0x0C, 0x10, 0x0A, 0x53, 0xD5, 0x40, 0x0A, 0x13, 0x03, 0x08, 0x03, 0x00}); // Set Gamma
    ILI9341_CMD(0xE1, {0x00, 0x00, 0x10, 0x03, 0x0F, 0x05, 0x2C, 0xA2, 0x3F, 0x05, 0x0E, 0x0C, 0x37, 0x3C, 0x0F}); // Set Gamma

    ILI9341_CMD(0x21, {}); // Exit Sleep

    ILI9341_CMD(0x11, {}); // Exit Sleep
    usleep(120 * 1000);      // Wait 5ms after sleep out
...
void rg_display_clear(uint16_t color_le)
{
    ...
    uint16_t led_color_be = (C_RED << 8) | (C_RED >> 8);
    for (size_t y = 0; y < RG_SCREEN_HEIGHT;)
    {
        ...
        for (size_t j = 0; j < buffer.length; ++j){
            if (j % RG_SCREEN_WIDTH < 35){
                buffer.ptr[j] = led_color_be;
            } else {
